...
func newAlohaTree(t *testing.T, db dbm.DB) (*iavl_v2.Tree, types.CommitID) {
	t.Helper()

	pool := iavl_v2.NewNodePool()
	opts := iavl_v2.TreeOptions{
		CheckpointInterval: 10, HeightFilter: 1, StateStorage: true, EvictionDepth: 8, MetricsProxy: metrics.NewStructMetrics(),
	}
	if _, ok := opts.MetricsProxy.(*metrics.StructMetrics); ok {
		opts.MetricsProxy = metrics.NewStructMetrics()
	}

	logger := slog.New(slog.NewTextHandler(os.Stdout, &slog.HandlerOptions{Level: slog.LevelDebug}))
	sql, err := iavl_v2.NewSqliteDb(pool, iavl_v2.DefaultSqliteDbOptions(iavl_v2.SqliteDbOptions{
		Path:    t.TempDir(),
	...
	tree := iavl_v2.NewTree(sql, pool, opts)
