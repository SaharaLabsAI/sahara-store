...
func (s *MigrateStoreTestSuite) SetupTest() {
	...
	orgSC, err := commitment.NewCommitStore(multiTrees, nil, mdb, testLog, metrics.NewNoOpMetrics())
	...
	sc, err := commitment.NewCommitStore(multiTrees1, nil, dbm.NewMemDB(), testLog, metrics.NewNoOpMetrics())
	s.Require().NoError(err)

	pm := pruning.NewManager(sc, nil)

	// assume no storage store, simulate the migration process
	s.rootStore, err = New(dbm.NewMemDB(), testLog, orgSC, pm, nil, metrics.NewNoOpMetrics())
	s.Require().NoError(err)
}
